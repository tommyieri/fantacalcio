...
${playersData}
    ...
    function inizializzaValutazioni() {
      ...
      for (const r of RUOLI) {
        ...
        roleStats[r] = { nDrafted, baselineFvm, roleBudget, excessSum, excessPool };
      }
      ...
      for (const p of PLAYERS) {
        ...
        const fma = Number((mv + bonusNet).toFixed(2));
    ...
    function calcolaPrezziEMioMax(st, mercato, asseg, and) {
      ...
      for (const p of PLAYERS) {
        ...
        // Proporzionalita' diretta col budget residuo e l'urgenza di reparto, SENZA doppio conteggio per tag TOP
        ...
        const rawMax = trueVal * Math.min(1.4, Math.max(0.65, budgetRatio)) * roleNeedBoost;
    ...
    function calcolaModificatoreSquadra(giocatoriDettaglio) {
      ...
      if (difensori.length < 3 || !portieri.length) {
        return { bonusAtteso: 0, scaglioneBase: 0, mvMedia: 0, stagionale: 0, topDif: difensori.slice(0, 3) };
      }
      ...
      // Scaglione Base Esatto (Regole Ufficiali Serie A Classic)
      let scaglioneBase = 0;
      if (media >= 7.00) scaglioneBase = 6.0;
      else if (media >= 6.75) scaglioneBase = 4.0;
      else if (media >= 6.50) scaglioneBase = 3.0;
      else if (media >= 6.25) scaglioneBase = 2.0;
      else if (media >= 6.00) scaglioneBase = 1.0;

      // Rendimento atteso su 38 giornate integrando la varianza voto partita (sigma = 0.28)
      const sigma = 0.28;
      const pGeq700 = 1 - normalCdf(7.00, media, sigma);
      const pGeq675 = 1 - normalCdf(6.75, media, sigma);
      const pGeq650 = 1 - normalCdf(6.50, media, sigma);
      const pGeq625 = 1 - normalCdf(6.25, media, sigma);
      const pGeq600 = 1 - normalCdf(6.00, media, sigma);

      const p6 = pGeq700;
      const p4 = pGeq675 - pGeq700;
      const p3 = pGeq650 - pGeq675;
      const p2 = pGeq625 - pGeq650;
      const p1 = pGeq600 - pGeq625;

      const expectedBonus = (1 * p1) + (2 * p2) + (3 * p3) + (4 * p4) + (6 * p6);
          ...
          radarHtml = \`
            <div class="w-full mt-3 pt-2.5 border-t border-slate-800 flex flex-col gap-2 text-xs">
              ...
              <div id="radar-rivali-list" class="flex flex-wrap gap-1.5">
                \${radar.inCorsa.map(r => \`<span class="px-2 py-0.5 rounded bg-slate-900 border border-slate-700 text-[11px] text-slate-300 flex items-center gap-1">
                  \${esc(r.nome)}: <strong class="text-white font-bold">\${r.maxBid}</strong> max
    ...
    function renderSimulatore(miaRosaPlayers, bestXI, modRes) {
      ...
      document.getElementById('sim-mod-mv').textContent = modRes.mvMedia > 0 ? \`\${modRes.mvMedia.toFixed(2)} (Scaglione +\${modRes.scaglioneBase} pt)\` : '0.00';
